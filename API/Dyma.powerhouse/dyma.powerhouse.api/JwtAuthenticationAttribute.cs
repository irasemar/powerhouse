...
namespace dyma.sit.api.Jwt
{
    public class JwtAuthenticationAttribute : Attribute, IAuthenticationFilter
    {
        ...
        private static bool ValidateToken(string token, out Models.UserSession username)
        {
            ...
            username.Nombres = usernameClaim?.Value;

            usernameClaim = identity.FindFirst(ClaimTypes.Surname);
            username.ApellidoPaterno = usernameClaim?.Value;

            usernameClaim = identity.FindFirst(ClaimTypes.NameIdentifier);
            username.ApellidoMaterno = usernameClaim?.Value;
            ...
            username.NFK_Planta = int.Parse(usernameClaim?.Value);
        ...
        protected Task<IPrincipal> AuthenticateJwtToken(string token)
        {
            ...
            if (ValidateToken(token, out usuario))
            {
                ...
                var claims = new List<Claim>
                {
                    ...
                    new Claim(ClaimTypes.NameIdentifier, usuario.ApellidoMaterno),
                    new Claim(ClaimTypes.Sid, usuario.NPK_Usuario.ToString()),
                    new Claim(ClaimTypes.PrimarySid, usuario.NFK_Planta.ToString()),
